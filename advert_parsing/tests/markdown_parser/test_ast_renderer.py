...
from advert_parsing.markdown_parser.md_ast import (
    StyleValue,
    Paragraph, Text, Heading, Listing, Table, Root, ListItem, Cell
)
 ...
 * *d*
 * ~~e~~ f''',
     Root(children=[Listing(children=[
            Text(text='a b c.'),
            Text(text='d'),
            ListItem(children=[
                Text(text='e', styles={StyleValue.STRIKE}), Text(text='f')
            ])
     ])])
     ),
    'blockquote':
...
''',
     Root(children=[
         Paragraph(children=[Text(text='a b c')])
         ])),
    'inline_code':
    (
        '''a `b`''',
...
''', 
        Root(children=[
            Paragraph(children=[Text(text="a b c d")])
        ])
    ),
...
table_test_cases = zip(
    tables_md,
    [tables_ast] * len(tables_md) 
)
...
NC-27587 >>>>SOLD<<<<'''
    expected = Root(children=[Paragraph(children=[
        Text(text='https://imgur.com/a/lMBDhmL Asking 315+Shipping, currently been mounted for 5 days now, 1 1/2 hours clocked of valorant. NC-27587 SOLD')
    ])])
    xml = parse_md_to_xml(md)
    ET.dump(xml)
    ast = xml_to_ast(xml, collapse=True)
