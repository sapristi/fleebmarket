...
from advert_parsing.markdown_parser import Text, LineBreak, ThematicBreak
...
def generate_curr_regexes(curr_exprs):
    ...
    for curr_expr in curr_exprs:
        if curr_expr == r'\\$':
            res.append(re.compile(f"{curr_expr} ?{number_group_regex}(?!(aud|cad|sgd))", re.I))
        else:
...
def find_prices_in_text(text: Text) -> list[PriceTag]:
    ...
    for curr, regexes in price_regexes.items():
        for regex in regexes:
            ...
            res.extend(PriceTag(currency=curr, amount=match.group('price'), striked=text.is_striked()) for match in matches)
    if not res:
        for regex in no_curr_price_regexes:
            matches = regex.finditer(text.text)
            res.extend(PriceTag(currency=None, amount=match.group('price'), striked=text.is_striked()) for match in matches)
...
def find_price_wo_curr_in_text(text: Text, min_amount=10):
    ...
    for match in matches:
        price_tag = PriceTag(currency=None, amount=match.group('price'), striked=text.is_striked())
