...
from .md_ast import (
    Root, Text, Table, Cell, Row, Paragraph, LineBreak, ThematicBreak,
...
def xml_to_ast_raw(xml) -> MdElement:
    ...
    if xml.tag == 'table':
        rows = [
            xml_to_ast_raw(row) for row in xml
        ]
...
def collapse_ast(ast: MdElement, classes_to_collapse):
    ...
    if all(isinstance(node, Text) for node in children):
        if all(node.styles == children[0].styles for node in children):
            ...
            if any(
                    isinstance(ast, klass) for klass in classes_to_collapse
            ):
                return Text(text=full_text, styles=children[0].styles)
            return ast.copy(
                update={
                "children": [Text(text=full_text, styles=children[0].styles)]
                }
            )

    return ast.copy(
        update={"children": [collapse_ast(child, classes_to_collapse) for child in children]}
    )


def xml_to_ast(xml: "Element", merge_style: bool = True, collapse: bool = False) -> MdElement:
    classes_to_collapse = []
