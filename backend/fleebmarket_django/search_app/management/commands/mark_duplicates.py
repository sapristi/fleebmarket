...
class Command(BaseCommand):
    def add_arguments(self, parser):
        parser.add_argument('--reset-duplicates', action="store_true", help="Set all duplicate status to False before applying")
        parser.add_argument('--wait', action="store_true", help="User input at each step")
    ...
    def handle(
        ...
        if reset_duplicates:
            with connection.cursor() as cursor:
                ...
                cursor.execute(f"UPDATE {RedditAdvert._meta.db_table} set is_duplicate = false")
