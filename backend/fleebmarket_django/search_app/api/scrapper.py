...
from ..schemas.reddit_advert import (
    RedditAdvertCreate,
    RedditAdvertDB,
    RedditAdvertDBLight,
    RedditAdvertUpdate,
)
from .deps import close_old_connections, meili_session, service_account
...
def get_to_update(nb: int = 100, min_score: float = 1.0):
...
def create_batch(
    ...
    for data_in in adverts:
        try:
            ...
            logger.warning(
                "Could not parse [%s], skipping: %s", data_in.get("reddit_id"), e
            )
            response.skipped.append(data_in.get("reddit_id", "no_reddit_id"))
            continue

        if (
            advert_in.ad_type is None
            or advert_in.ad_type in models.RedditAdvertType.ignored()
        ):
            continue
        ...
        try:
            extra = models.parse_mechmarket_advert(
                advert_in.ad_type, advert_in.title, advert_in.full_text
            )
        except Exception as e:
            logger.warning(
                "Could not parse [%s], skipping: %s", data_in.get("reddit_id"), e
        ...
        if extra is None:
            logger.warning(
                "Advert %s will be skipped (%s)", advert_in.reddit_id, advert_in.title
            )
            response.skipped.append(advert_in.reddit_id)
            continue

        advert_db = models.RedditAdvert(**advert_in.dict(), extra=extra)
        try:
            advert_db.save()
        except Exception as exc:
...
def update(
    ...
    for advert in adverts:
        if advert.ad_type is None:
            ...
            continue
        ad_db: models.RedditAdvert = models.RedditAdvert.objects.get(
...
def mark_updated(advert_ids: list[int]):
