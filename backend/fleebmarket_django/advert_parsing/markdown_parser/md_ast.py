...
ChildrenT = TypeVar(
    "ChildrenT", PhrasingContent, FlowContent, Union[FlowContent, PhrasingContent]
)
...
class Parent(GenericModel, Generic[ChildrenT]):
    ...
    def apply(self: ParentT, fun) -> ParentT:
        return self.copy(update={"children": [fun(child) for child in self.children]})
...
class Table(BaseModel):
    ...
    def tranform_rows(cls, v):
        res = [row.children if isinstance(row, Row) else row for row in v]
        return res
    ...
    def apply(self, function):
        return Table(rows=[[function(cell) for cell in row] for row in self.rows])
    ...
    def to_html(self):
        ...
        inner_html = "".join((f"<tr>{row_html}</tr>" for row_html in rows_html))
