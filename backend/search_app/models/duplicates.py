...
def tokenize_groups(terms):
    ...
    return [token.strip(" ").lower() for token in tokens if len(token.strip(" ")) > 0]
...
def tokenize_words(terms):
    return [t.lower() for t in terms.split() if len(t) > 2]
...
def duplicate_offers(terms1, terms2):
    return (
        similar_groups(terms1, terms2) >= MIN_SIMILAR_GROUPS
        or similar_fuzz(terms1, terms2) >= MIN_FUZZ_SIMILARITY
        or similar_words(terms1, terms2) >= MIN_SIMILAR_WORDS
    )
