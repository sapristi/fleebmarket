...
app = typer.Typer(help="collects and send alerts ", no_args_is_help=True)
...
class AlertEntry:
    ...
    def format_terminal(self, count) -> str:
        return (
            typer.style(f"[{self.service}]: {count} occurences\n", bold=True)
            + self.message
        )
...
class AlertsCollector:
    ...
    def __init__(self):
        services_ignored_file = Path(os.environ['DATA_PATH']) / "alerts"/ "services_ignored"
        messages_ignored_file = Path(os.environ['DATA_PATH']) / "alerts"/ "messages_ignored"
    ...
    def is_entry_ignored(self, entry: AlertEntry):
        return (
            entry.service in self.services_ignored or
            any((message_ignored in entry.message) for message_ignored in self.messages_ignored)
        )

    def collect_journal_messages(
        self,
        since: datetime,
        journal_mode: JournalOpenMode,
        rule: Rule
    ):
    ...
    def collect_services_alerts(
        ...
        self.alerts.update((AlertEntry(message, service, 3) for (service, message) in errors))
...
@app.command(help="print collected alerts to stdout")
def show(since_hours: int = 1):
...
@app.command(help="send collected alters to the discord #alerts channel")
def send(
    discord_token: str = typer.Argument(..., envvar='DISCORD_BOT_TOKEN'),
